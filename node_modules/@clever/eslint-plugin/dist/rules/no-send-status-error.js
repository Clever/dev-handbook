...
exports.default = shared_1.createRule({
    ...
    meta: {
        ...
        messages: {
            noSendStatus: "using res.sendStatus with an error status code is not allowed. You should throw an exception instead, which the global error handler will handle.",
            noStatus: "using res.status with an error status code is not allowed. You should throw an exception instead, which the global error handler will handle.",
        },
        schema: [],
    },
    defaultOptions: [],
    create(context) {
        ...
        return {
            ":matches(CallExpression, OptionalCallExpression)[callee.property.name=sendStatus][arguments.0.value >= 400]"(node) {
                context.report({
                    node,
                    messageId: "noSendStatus",
                });
            },
            ":matches(CallExpression, OptionalCallExpression)[callee.property.name=status][arguments.0.value >= 400]"(node) {
                context.report({
                    node,
                    messageId: "noStatus",
                });
